...
"""
                ...
                bat label: 'Send Telegram Message', script: "curl -X POST -H \"Content-Type: application/json\" -d \"{\\\"chat_id\\\":\\\"${env.CHAT_ID}\\\", \\\"text\\\": \\\"${message}\\\", \\\"parse_mode\\\": \\\"Markdown\\\", \\\"disable_notification\\\": false}\" https://api.telegram.org/bot${env.TOKEN}/sendMessage"
            }
        }
        failure {
            script {
